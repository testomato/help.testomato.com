...
:::tip **String checks** are case sensitive, so make sure you **PaY ClosE AtTentioN** to the text you enter. :::

### How regular expressions are processed

Testomato processes and escapes all users inputs including regular expressions. We are using `{` and `}` for wrapping your regex and also adding the following [Pattern Modifiers](https://www.php.net/manual/en/reference.pcre.pattern.modifiers.php):
...
:::note Need to check structure or counts instead of text — e.g. exactly one `<h1>`, or every `<img>` has `alt` text? See [XPath checks](xpath). :::
